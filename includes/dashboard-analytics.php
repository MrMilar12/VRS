...
function dashboard_analytics(
    ...
    $today = new DateTimeImmutable(substr($now, 0, 10));
    $start =
        $period === 'all'
            ? null
            : $today
                ->modify('first day of this month')
                ->modify('-' . ((int) $period - 1) . ' months');
    $result = [
        'total' => 0,
        'vehicle' => 0,
        'personnel' => 0,
    ...
    foreach (['vehicle' => $vehicles, 'personnel' => $personnel] as $type => $records) {
        ...
        foreach ($records as $r) {
            ...
            if (
                ...
                ($start && $created < $start->format('Y-m-d'))
            ) {
                continue;
            }
            $result['total']++;
            ...
            $result['offices'][$office] = ($result['offices'][$office] ?? 0) + 1;
            $month = substr($created, 0, 7);
            $result['months'][$month] ??= ['vehicle' => 0, 'personnel' => 0];
            $result['months'][$month][$type]++;
        }
    }
    $first = $start?->format('Y-m');
    if (!$first && $result['months']) {
        $first = min(array_keys($result['months']));
    }
    $first ??= $today->format('Y-m');
    for (
        $month = new DateTimeImmutable($first . '-01');
        $month <= $today;
        $month = $month->modify('+1 month')
    ) {
        $result['months'][$month->format('Y-m')] ??= ['vehicle' => 0, 'personnel' => 0];
    }
    ksort($result['months']);
    arsort($result['statuses']);
    arsort($result['offices']);
    return $result;
}
...
<div class="analytics-bar-row">
    ...
    <div class="analytics-track" aria-hidden="true">
        <span class="<?= e(
            $tone,
        ) ?>" style="width:<?= ($value / $max) * 100 ?>%"></span>
    </div>
</div>
<?php endforeach;
}
