 ...
 <section class="panel booking-chat" aria-labelledby="booking-chat-title">
  ...
  <div class="assistant-welcome" data-assistant-welcome <?=empty($state['messages'])?'':'hidden'?>>
   ...
   <div class="assistant-prompts" aria-label="Suggested questions">
    <?php foreach([['car','Find a vehicle','See what’s available','Which vehicles are available now?'],['users','Find a driver','Check driver availability','Which drivers are available now?'],['file','Plan my trip','Prepare a vehicle request','Help me book a vehicle.'],['shield','Understand approvals','Who approves my request?','How does approval work?'],['users','Request personnel','Get help with staff requests','How do I request personnel?'],['search','Track a request','Find status and progress','How do I track my request?']] as [$ico,$title,$caption,$prompt]):?><button type="button" data-assistant-prompt="<?=e($prompt)?>"><span class="assistant-prompt-icon"><?=icon($ico,20)?></span><strong><?=e($title)?></strong><small><?=e($caption)?></small></button><?php endforeach?>
   </div>
  </div>
