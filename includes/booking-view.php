...
page_heading('Booking assistant','Plan a trip, ask a question, or get help with vehicle and personnel requisitions.','<a class="btn" href="index.php?page=create">Create request manually</a>');
?>
<div class="booking-layout <?=$showReview?'':'booking-chat-only'?>" data-booking-assistant>
 <section class="panel booking-chat" aria-labelledby="booking-chat-title">
  <div class="panel-heading assistant-chat-heading"><div class="assistant-identity"><span class="assistant-symbol" aria-hidden="true"><?=icon('car',22)?></span><div><h2 id="booking-chat-title">How can I help?</h2><p>VPRS booking & help assistant</p></div></div><button class="btn small assistant-reset" type="button" data-booking-reset>New chat</button></div>
  <?php if(!$connected):?><div class="booking-notice" role="status"><strong>AI booking is not connected yet</strong><p><?=e(booking_configuration_error())?></p><p>Basic help still works here. For trip preparation and live availability, use the regular forms while your administrator completes the connection.</p></div><?php endif?>
  <div class="assistant-welcome" data-assistant-welcome <?=empty($state['messages'])?'':'hidden'?>>
   ...
   <p>Tell me what you need, even if you only know a few details.<br>You can also ask about approvals, personnel requests, or tracking.</p>
   <div class="assistant-prompts" aria-label="Suggested questions">
    <?php foreach([['car','Find a vehicle','See what’s available','Which vehicles are available now?'],['users','Find a driver','Check driver availability','Which drivers are available now?'],['file','Plan my trip','Prepare a vehicle request','Help me book a vehicle.'],['shield','Understand approvals','Who approves my request?','How does approval work?'],['users','Request personnel','Get help with staff requests','How do I request personnel?'],['search','Track a request','Ask about a slip','I want to track a request.']] as [$ico,$title,$caption,$prompt]):?><button type="button" data-assistant-prompt="<?=e($prompt)?>"><span class="assistant-prompt-icon"><?=icon($ico,20)?></span><strong><?=e($title)?></strong><small><?=e($caption)?></small></button><?php endforeach?>
   </div>
  </div>
 ...
 <section class="assistant-availability" data-availability-panel hidden aria-labelledby="availability-heading"><span class="assistant-eyebrow">FLEET AT A GLANCE</span><h3 id="availability-heading">Your availability results</h3><p data-availability-summary></p><button type="button" class="btn small" data-availability-refresh>Refresh availability</button><p class="field-hint" data-availability-checked role="status"></p><div data-availability-items></div></section>
  <div class="assistant-thinking" data-assistant-thinking hidden role="status"><span class="assistant-thinking-dots"><i></i><i></i><i></i></span><span>VPRS assistant is thinking…</span></div>
  </div>
 ...
 <label>Vehicle type<select name="vehicle_type" required><option value="">Choose a vehicle type</option><?php foreach(['Van','MPV','SUV','Pickup','Sedan','Bus','Motorcycle'] as $type):?><option <?=($draft['vehicle_type']??'')===$type?'selected':''?>><?=$type?></option><?php endforeach?></select></label>
