...
function work_reminders(array $vehicles,array $personnel,string $now,int $viewerId): array {
 $today=substr($now,0,10);$items=[];
 foreach(['Vehicle'=>$vehicles,'Personnel'=>$personnel] as $kind=>$records)foreach($records as $r){
  $status=$r['status'];
  ...
  $task=match($status){
   'Draft'=>'Submit draft', 'Returned for Correction'=>'Revise request',
   'Pending Supervisor','Pending Administrative Approval'=>'Awaiting approval',
   'Approved'=>$late?'Review missed schedule':($kind==='Personnel'?'Needs start':'Needs dispatch'),
