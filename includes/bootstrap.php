...
if ((@filemtime($updateMarker) ?: 0) > time() - 300) {
    http_response_code(503);
    header('Retry-After: 30');
    exit('A software update is being installed. Please try again shortly.');
}
...
try {
    $pdo = new PDO(
        $config['demo'] ? 'sqlite:' . __DIR__ . '/../storage/demo.sqlite' : $config['dsn'],
        $config['username'],
        $config['password'],
        [
    ...
    http_response_code(503);
    exit(
        'Database unavailable. <a href="setup.php">Open workspace setup</a> or follow README.md to configure MySQL.'
    );
}
// Bind authentication to its database; demo IDs cannot carry into a new installation.
$databaseIdentity = hash(
    'sha256',
    ($config['demo'] ? 'demo-sqlite' : $config['dsn']) . '|' . $config['username'],
);
...
if (!empty($_SESSION['user_id'])) {
    ...
    $valid = auth_session_valid(
        $account,
        $_SESSION,
        null,
        $account && auth_is_disabled((int) $account['id']),
    );
    if ($valid) {
        $user = $account;
        $_SESSION['auth_seen'] = time();
    } else {
        unset($_SESSION['user_id'], $_SESSION['auth_level'], $_SESSION['auth_credential']);
        session_regenerate_id(true);
        $_SESSION['csrf'] = bin2hex(random_bytes(32));
    }
}
