...
function booking_validate(array $raw): array {
 ...
 if($draft['vehicle_type']!==''&&!in_array($draft['vehicle_type'],['Van','MPV','SUV','Pickup','Sedan','Bus','Motorcycle']))throw new RuntimeException('The assistant returned an unsupported vehicle type.');
...
function booking_followup(array $validated): string {
 ...
 $prefix=$draft['destination']!==''?'Let’s plan your trip to '.$draft['destination'].'. ':'I can help you prepare a vehicle request. ';
 $questions=['destination'=>'Where would you like to go?','start_datetime'=>'What date and time will you leave?','end_datetime'=>'When do you expect to return?','vehicle_type'=>'What type of vehicle do you need?','passengers'=>'Who will be travelling? Please give their names.','purpose'=>'What is the purpose of the trip?'];
 foreach($questions as $key=>$question)if($draft[$key]==='')return $prefix.$question;
...
function booking_payload(array $messages,array $draft): array {
 ...
 $properties['vehicle_type']=['type'=>['string','null'],'enum'=>['Van','MPV','SUV','Pickup','Sedan','Bus','Motorcycle',null]];
...
function booking_decode_reply(string $output): array {
 ...
 if(strlen($output)>65536)throw new BookingResponseFormatException('The assistant returned an oversized reply. Please try a shorter question.');
 // Some providers wrap the JSON in Markdown, or add a short introduction.
 if(preg_match('/^```(?:json)?\s*(.*?)\s*```$/is',$output,$match))$output=trim($match[1]);
 try{$decoded=json_decode($output,true,32,JSON_THROW_ON_ERROR);}
 catch(JsonException $e){
  $start=strpos($output,'{');$end=null;$depth=0;$quoted=false;$escaped=false;
  if($start!==false)for($i=$start,$length=strlen($output);$i<$length;$i++){
   $char=$output[$i];
   if($quoted){if($escaped)$escaped=false;elseif($char==='\\')$escaped=true;elseif($char==='"')$quoted=false;continue;}
   if($char==='"'){$quoted=true;continue;}
   if($char==='{')$depth++;elseif($char==='}'&&--$depth===0){$end=$i;break;}
  }
  if($end===null||str_contains(substr($output,$end+1),'{'))throw new BookingResponseFormatException('The assistant could not format its reply. Please try again.');
  try{$decoded=json_decode(substr($output,$start,$end-$start+1),true,32,JSON_THROW_ON_ERROR);}
  catch(JsonException $e){throw new BookingResponseFormatException('The assistant could not format its reply. Please try again.');}
 }
 if(!is_array($decoded)||array_is_list($decoded))throw new BookingResponseFormatException('The assistant returned an incomplete reply. Please try again.');
 return $decoded;
}
function booking_respond(array $messages,array $draft): array {
 for($attempt=0;$attempt<2;$attempt++){
  try{return booking_provider_respond($messages,$draft);}
  catch(BookingResponseFormatException $e){
   if($attempt===1)throw new RuntimeException('The assistant could not format a reply after retrying. Your saved booking details are unchanged. Please try rephrasing your question.');
   $messages[]=['role'=>'user','content'=>'Formatting correction: answer the previous user question using exactly one valid JSON object matching the required schema. Include intent, topic, lookup, reply, and draft. Do not include Markdown fences, commentary outside JSON, or trailing commas. Preserve the existing draft and do not invent missing information.'];
  }
 }
...
function booking_parse_response(array $response): array {
 ...
 $output='';foreach($response['output']??[] as $item)foreach($item['content']??[] as $part){if(($part['type']??'')==='refusal')throw new RuntimeException('Please ask about a vehicle trip or how to use VPRS so I can help.');if(($part['type']??'')==='output_text')$output.=$part['text']??'';}
 $result=booking_decode_reply($output);
 if(!is_array($result)||!is_array($result['draft']??null)||!is_string($result['reply']??null)||mb_strlen($result['reply'])>2000)throw new BookingResponseFormatException('The assistant returned an incomplete reply. Please try again.');
 $intent=$result['intent']??'booking';$topic=$result['topic']??'overview';
 if(!in_array($intent,['booking','vehicles','drivers','system','conversation','tracking'])||!in_array($topic,['overview','approval','booking','editing','authenticator','calendar','dispatch','profile','availability','personnel','tracking','notifications','registration','reminders'])||!is_array($result['lookup']??[]))throw new BookingResponseFormatException('The assistant could not understand that request. Please try again.');
 ...
 $actionClaim=preg_match('/\b(submitted|approved|reserved|cancelled|canceled|deleted|dispatched|na-submit|na-approve|na-reserve)\b/iu',$reply);
 if($reply===''||$actionClaim)$reply=$validated['ready']?'Your trip details are ready to review.':booking_followup($validated);
 if($intent==='booking')$reply.=$validated['ready']?"\n\nReview the form and select Submit request for approval. Nothing has been submitted yet.":"\n\nYour draft is saved in this conversation; nothing has been submitted yet.";
