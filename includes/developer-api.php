...
if($mode==='check'){echo json_encode($updater->check('',true),JSON_THROW_ON_ERROR);return;}
...
try{audit($mode==='rollback'?'Code update rolled back':'Code update installed',$result['previous'].' → '.$result['current']);}catch(Throwable $e){error_log('VRS code deployment succeeded, but its audit entry could not be saved.');$result['message'].=' The audit entry could not be saved; check the server log.';}
