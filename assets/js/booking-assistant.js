...
(() => {
 ...
 let busy=false,refreshing=false,availabilityActive=false,conversationVersion=0;
 function showAvailability(data,host=null){
  if(!data){const wasActive=availabilityActive;availabilityActive=false;refresh.hidden=true;if(wasActive&&!availabilityPanel.hidden)root.querySelector('[data-availability-checked]').textContent+=' · Previous result; ask again for a fresh check.';return;}
  availabilityActive=!data.needs_dates;
  if(host){const previous=availabilityPanel.closest('.booking-message');if(previous&&previous!==host){const summary=document.createElement('p');summary.textContent=root.querySelector('[data-availability-summary]').textContent;previous.append(summary);previous.classList.remove('availability-message');}host.querySelector('p')?.remove();host.append(availabilityPanel);host.classList.add('availability-message');}
  const opening=availabilityPanel.hidden;togglePanel(availabilityPanel,true);
 ...
 async function refreshAvailability(){if(busy||refreshing||!availabilityActive)return;const version=conversationVersion;refreshing=true;refresh.disabled=true;try{const result=await request({mode:'refresh'});if(version===conversationVersion)showAvailability(result.availability);}catch(error){if(version===conversationVersion)root.querySelector('[data-availability-checked]').textContent=error.message+' Displayed results may be out of date.';}finally{refreshing=false;refresh.disabled=false;}}
 ...
 function bubble(role,text){root.querySelector('[data-assistant-welcome]').hidden=true;messages.hidden=false;const node=document.createElement('div');node.className='booking-message '+role;const title=document.createElement('strong');title.textContent=role==='user'?'You':'VPRS assistant';const p=document.createElement('p');p.textContent=text;node.append(title,p);messages.append(node);reveal(node);messages.scrollTo({top:messages.scrollHeight,behavior:reducedMotion.matches?'auto':'smooth'});return node;}
