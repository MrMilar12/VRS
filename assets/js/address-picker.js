...
(() => {
  function initialize(picker) {
    if (picker.dataset.addressReady) return;
    picker.dataset.addressReady = 'true';
    ...
    const cache = new Map();
    let results = value.value ? [{ label: value.value }] : [],
      active = -1,
      timer,
      controller,
      generation = 0,
    ...
    function choose(place) {
      ...
      const params = new URLSearchParams({
        bbox: [
          ...
          Math.max(-90, lat - 0.01),
          Math.min(180, lon + 0.015),
          Math.min(90, lat + 0.01),
        ].join(','),
        layer: 'mapnik',
        marker: `${lat},${lon}`,
      });
    ...
    async function search(term, version) {
      ...
      try {
        ...
        if (!places) {
          lastSearch = Date.now();
          const url = new URL(picker.dataset.searchUrl);
          url.search = new URLSearchParams({ q: term, limit: '8', lang: 'en' });
          const response = await fetch(url, { signal: request.signal, credentials: 'omit' });
          if (!response.ok) throw new Error('Search unavailable');
          const data = await response.json();
          if (!Array.isArray(data.features)) throw new Error('Invalid results');
          const seen = new Set();
          places = [];
        ...
        status.textContent = places.length
          ? `${places.length} places found. Select a place.`
          : 'No places found. You can use the destination you typed.';
        render();
        if (places.length && !list.hidden) highlight(0);
      } catch (error) {
        if (version !== generation) return;
        results = [];
        status.textContent = 'Search unavailable. You can still use the destination you typed.';
        render();
      } finally {
    ...
    input.addEventListener('input', () => {
      ...
      status.textContent =
        term.length < 3 ? 'Type at least 3 characters to find a place.' : 'Searching places…';
      render();
      open(true);
      if (term.length >= 3)
        timer = setTimeout(
          () => search(term, version),
          Math.max(450, 1000 - (Date.now() - lastSearch)),
        );
    });
    ...
    picker.addEventListener('focusout', (event) => {
      if (!picker.contains(event.relatedTarget)) open(false);
    });
    render();
  }
  document.querySelectorAll('[data-address-picker]').forEach(initialize);
  document.addEventListener('destinations:add', (event) =>
    event.detail.querySelectorAll('[data-address-picker]').forEach(initialize),
  );
})();
