...
if($_SERVER['REQUEST_METHOD']==='POST')try{
 ...
 if(!one('SELECT id FROM users WHERE email=?',[$email])){
  try{run('INSERT INTO users(full_name,email,password_hash,role,office_id,position,status) VALUES(?,?,?,?,?,?,?)',[$name,$email,$hash,'Requester',$office,$position,'Pending']);$id=(int)$pdo->lastInsertId();audit('Registration requested','Pending requester account #'.$id);
   foreach(all("SELECT id FROM users WHERE role='Administrator' AND status='Active'") as $admin)run('INSERT INTO notifications(user_id,message,created_at) VALUES(?,?,?)',[$admin['id'],'A new account is awaiting review on the Account requests page.',date('Y-m-d H:i:s')]);
