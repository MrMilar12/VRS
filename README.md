...
- Fleet records including validated, re-encoded JPEG/PNG vehicle photographs; drivers, offices, users, roles, and account activation. Unused records can be deleted with confirmation; records linked to operational history can be set to Inactive.
...
Use **Personnel** under management to add staff, their position/specialty, office, and availability (Available, On Leave, or Inactive). Choose **Driver** or **Utility** classification. Drivers require a license number and expiration date. Both classifications share this list; login accounts remain separate. Existing drivers are imported automatically, and old Drivers links open Personnel. Existing trip references and personnel requisitions are preserved.

Use **Personnel requisitions → New personnel requisition** to save a draft or submit an assignment request with a role, optional preferred personnel name, location, duties, and schedule. Preferred personnel is shown in requisition details and printed slips; the administrator still confirms the assignment and availability. Submitted personnel requisitions appear alongside vehicle requests in **Approvals**, including the pending count. The administrator opens a requisition, selects personnel from a dropdown, uses **Add personnel** for additional staff (or **Remove** to remove a selection), and confirms approval with their password, or returns/rejects the request with remarks. Requesters can edit drafts/returned requests and cancel requisitions before they start. Administrators and dispatchers start approved assignments within their scheduled period and mark them complete. Approved requisitions have a printable slip; the list supports date, status, and text filters.

Availability checks cover both vehicle trips and personnel requisitions for the same staff member. Overlapping approvals, the configured turnaround buffer, leave/inactive status, and overdue assignments in progress block assignment. Approval rechecks every selected person under resource locks and saves all assignments together. Details, lists, and printed slips show all assigned personnel. Starting an assignment rechecks everyone; completion and cancellation release the whole group. Existing single-person assignments are preserved automatically. Notifications and requisition history record workflow updates. Personnel can be managed by administrators and administrative officers; requisition visibility follows existing requester/office permissions.

Existing installations create the additive personnel tables on application startup, using `database/personnel.mysql.sql` or `database/personnel.sqlite.sql`. MySQL accounts need table-creation permission for this first run (or an administrator can apply `database/personnel.mysql.sql` beforehand). Fresh MySQL installation includes these tables. The startup migration also adds classification and license fields and imports existing drivers. Matching employee numbers are combined into one personnel record; existing personnel details are retained. Driver IDs are retained internally for historical trip references. MySQL accounts need ALTER permission for upgrades.
