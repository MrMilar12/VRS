...
The booking assistant defaults to **Ollama Cloud**, using `gpt-oss:120b-cloud` through your local Ollama server at `http://127.0.0.1:11434`. Run `ollama signin` to connect your Ollama account and `ollama pull gpt-oss:120b-cloud` to add the cloud model entry. Keep Ollama running on the same computer as PHP/XAMPP. Inference runs in Ollama Cloud; the local server handles authentication and forwarding. No OpenAI API key is needed. Phones connect to VRS as usual and do not need Ollama installed.

Override the server or model using `OLLAMA_URL` and `OLLAMA_MODEL` in the PHP server environment, or `ollama_url` and `ollama_model` entries in the existing return array in `config/local.php`. The URL is the server root (without `/api/chat`). To return to local inference, set `ollama_model` to an installed local model such as `llama3.2:latest`. For an Ollama server on another computer, configure that server's reachable address. Preserve all existing database settings. Restart PHP/Apache after changing environment variables.
...
Checks: `php tests/booking-assistant.php` and `python3 tests/smoke.py`. Live AI checks require the configured provider to be running; only OpenAI mode requires an API key. Offline tests do not contact either provider.
...
Administrators can open **Developer center** in the sidebar. It checks `origin/main` in `MrMilar12/VRS` when opened and every 60 seconds while visible. The page displays **New update available**, a browser-tab indicator, commit IDs, and a changed-file preview. Checks fetch Git metadata only; they never install code automatically. Push notifications/webhooks and unattended background checking are not configured; the next open or poll detects a push.

Commit and push this developer-page implementation first. Deploy from a clean Git checkout at the repository root. The PHP server account needs Git, a PHP CLI binary, write access to the code, `.git` and `storage`, and permission to fetch the configured GitHub repository. Configure private-repository credentials for that server account outside the application; the web page does not collect GitHub tokens or show command output. If Git reports ownership issues, configure the exact checkout as trusted for the service account after verifying its owner. Never set Git safe.directory to a wildcard.

Settings in `config/system.php` (override in `config/local.php`): `update_repository`, `update_branch`, `update_git_binary`, and `update_php_binary`. Executable paths can also use `VRS_GIT_BINARY` and `VRS_PHP_BINARY`. For XAMPP, the CLI may be `/Applications/XAMPP/xamppfiles/bin/php`; the Git executable must be available to Apache. The configured repository must match `origin` exactly. Requests cannot supply another repository, branch or command.

**Apply update** requires an Administrator password and an explicit version confirmation. The updater rejects dirty/untracked code, a different branch, divergent histories, symlinks, submodules, and any remote tree that tracks runtime files. It syntax-checks all PHP files using the configured CLI before fast-forwarding to the exact previewed commit. Applying replaces tracked application code, including added/deleted files; it preserves ignored `config/local.php`, `storage` data, and uploads. Code changes must be committed rather than discarded. The current development workspace may therefore show a blocker until its changes are committed and pushed.

Before applying, a backup reference is created under `refs/vrs/backups/`; metadata is stored in `storage/update-deployment.json`. **Roll back previous update** restores the prior code with `git reset --keep` only while the installed version is unchanged and the working tree is clean. This is a code checkpoint, not a database backup. Database migration scripts are not run automatically; releases requiring schema migrations need a separately planned deployment and database backup. Files already in use by existing requests may finish with the previous code; new requests see a short maintenance response during file replacement. If the updater process is interrupted, its maintenance marker expires after five minutes. Inspect Git status before retrying; a backup reference remains available for manual recovery if the application cannot load.

Validation: `php tests/updater.php` exercises preview, clean-checkout protection, stale previews, locking, replacement, rollback, protected files, and PHP linting in a disposable repository. `python3 tests/smoke.py` checks Administrator-only access, password/CSRF enforcement and page rendering without fetching GitHub or updating application code.
