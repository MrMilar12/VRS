...
expect(
    $config['dsn'] === 'mysql:host=127.0.0.1;port=3306;dbname=vrs_test;charset=utf8mb4',
    'MySQL DSN constructed',
);
expect($config['password'] === $input['db_password'], 'Database password preserved literally');
foreach (
    [
        'db_host' => 'localhost;dbname=other',
        'db_name' => 'vrs;DROP DATABASE x',
        'db_port' => '0',
        'db_user' => '',
    ]
    as $key => $bad
    ...
    $rejected = false;
    try {
        installation_input(array_replace($input, [$key => $bad]));
...
expect(
    password_verify($input['admin_password'], $account['password_hash']),
    'Administrator password hashed',
);
$rejected = false;
try {
    installation_account(array_replace($input, ['confirm_password' => 'mismatch']));
} catch (RuntimeException $e) {
    $rejected = true;
}
...
expect(
    $row['role'] === 'Administrator' &&
        $row['status'] === 'Active' &&
        $row['email'] === $input['admin_email'],
    'Seed active administrator',
);
expect(
    (int) $db->query('SELECT COUNT(*) FROM users')->fetchColumn() === 1,
    'No demo accounts created',
);
expect(
    $db
        ->query("SELECT setting_value FROM system_settings WHERE setting_key='organization'")
        ->fetchColumn() === $input['organization'],
    'Organization settings saved',
);
expect(
    (int) $db->query('SELECT COUNT(*) FROM audit_logs')->fetchColumn() === 1,
