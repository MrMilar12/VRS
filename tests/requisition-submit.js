...
async function checkSave(success) {
  ...
  class Form {
    constructor() {
      ...
      this.dataset = {};
      this.button = { name: 'submit_mode', value: 'draft', disabled: false };
    }
    matches() {
      return true;
    }
    getAttribute(name) {
      return name === 'action' ? 'actions.php' : null;
    }
    querySelector(selector) {
      if (selector === '[data-save-status]') return notice || null;
      return { before: (node) => (notice = node) };
    }
    querySelectorAll() {
      return [this.button];
    }
  ...
  class Data extends Map {
    constructor(form) {
      super(Object.entries(form.values));
    }
  }
  const form = new Form();
  const document = {
    baseURI: 'http://localhost/vrs/index.php?page=create',
    querySelector: () => null,
    querySelectorAll: () => [],
    addEventListener: (type, fn) => {
      if (type === 'submit') handler = fn;
    },
    createElement: () => ({ dataset: {}, setAttribute() {}, focus() {} }),
  };
  vm.runInNewContext(source, {
    ...
    fetch: async (url, options) => {
      ...
      return {
        ...
        json: async () =>
          success
            ? { redirect: 'index.php?page=request&id=v1_fixture' }
            : { error: 'Please correct the return time.' },
      };
    },
  });
  const event = {
  ...
  assert.equal(
    request.url,
    'http://localhost/vrs/actions.php',
    'Named action input must not become the URL',
  );
  ...
  if (success)
    assert.equal(navigation, 'http://localhost/vrs/index.php?page=request&id=v1_fixture');
  else {
    assert.equal(navigation, undefined);
    assert.equal(notice.textContent, 'Please correct the return time.');
  }
  console.log(
    'PASS: Shadowed form.action posts to actions.php; ' +
      (success ? 'successful save opens record' : 'failed save keeps entered data'),
  );
}
(async () => {
  await checkSave(false);
