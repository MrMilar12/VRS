...
try{
 ...
 if($action==='delete_record'){
  ...
  $values=[$office,$type,field('preferred_driver',160,false),field('passengers',5000),$start,$end,destination_value(),field('purpose',5000),isset($_POST['fuel_allocation'])?1:0,$fuel,field('fuel_remarks',1000,false),$status];
  ...
  run('UPDATE users SET status=?,role=? WHERE id=?',[$decision==='approve'?'Active':'Rejected','Requester',$id]);
  audit('Registration '.($decision==='approve'?'approved':'rejected'),'Account #'.$id);flash($decision==='approve'?'Account approved and added to Users & roles. The requester can now sign in.':'Account request rejected.');$target='index.php?page=account-requests';
